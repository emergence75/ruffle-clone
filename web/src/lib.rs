...
use web_sys::{
    AddEventListenerOptions, ClipboardEvent, Element, Event, EventTarget, FocusEvent, Gamepad,
    GamepadButton as WebGamepadButton, HtmlCanvasElement, HtmlElement, KeyboardEvent, Node,
    PointerEvent, WheelEvent, Window,
};
        ...
        let _ = self.with_instance_mut(|instance| {
            ...
            if let Ok(gamepads) = instance.window.navigator().get_gamepads() {
                if let Some(gamepad) = gamepads.into_iter().next() {
                    let gamepad: Gamepad = gamepad.dyn_into().unwrap();

                    let mut pressed_buttons = Vec::new();

                    let buttons = gamepad.buttons();
                    for (index, button) in buttons.into_iter().enumerate() {
                        let button: WebGamepadButton = button.dyn_into().unwrap();
                        if !button.pressed() {
                            continue;
                        }
